# Taller modificadores de acceso
...
2) Salida del programa
    ...
    >False True
    >>``hasattr(ojt, nombre)`` indica si es verdadero o falso que existe el nombre del atributo mencionado.
...
6) Identifica el error
    ...
    >Un error porque no se puede definir el atributo ``y`` porque no está incluido en la tupla de ``__slots__``
    >>``__slots__`` es una tupla conn todos los atributos posibles de crear. Atributos que no se incluyan en la lista no podrán ser agregados.
...
10) Comprensión de dir y mangling
    ...
    >>``dir(objeto)`` muestra una lista con todos los atibutos y métodos de un objeto.
    >>``objeto.__dict__`` da un diccionario con todos los atributos de un objeto.
    >>``dir(type(obj))`` muestra los métodos
